    ...
    try:
        ...
        if ver<gitver:
            ...
            if '\n 'in gitreturn['body']:
                ...
                hh=''
            IsGotoupd=tkinter.messagebox.askyesno("Info",'有新版本可用！建议及时更新~\n当前版本：'+str(version)+'\n最新版本：'+str(gitversion)+'\n更新说明：'+hh+gitreturn['body']+'\n点击“是”前往更新，点击“否”继续运行')
            if IsGotoupd:
...
def linktodyid(dyid):
    ...
    if "t.bilibili.com/" in dyid:
        ...
        r=requests.get("https://bili.fan/api/checkjump.php?link="+dyid,verify=False)
        dyid=r.text
        #printp(dyid)
        dyid, sep, tail = dyid.partition('?')
        dyid=dyid[dyid.rfind('/'):]
...
def clicked0():
    ...
    if True:#chk6_state.get():
        ...
        output['state']='disabled'
    if txt.get()=='':
        tkinter.messagebox.showwarning("Info",'需要输入动态链接/ID的嗷！')
        return False
    try:
        ...
        tkinter.messagebox.showwarning("Info",'输入的动态ID/链接不正确呢！')
        #printp('')
        return False
    if len(str(dyid))<18:
        tkinter.messagebox.showwarning("Info",'输入的动态ID长度不够呢 ('+str(len(str(dyid)))+'/'+'18) ！')
        return False
    LBGZ=[]
    LBZF=[]
    LBPL=[]
    ...
    if not TGZ and not TZF and not TPL and not TDZ:
        tkinter.messagebox.showwarning("Info",'需要至少选中一个获奖条件呢！')
        #printp()
        return False
    if TGZ and not TZF and not TPL and not TDZ:
        tkinter.messagebox.showwarning("Info" '还需要选择除了关注以外的任一获奖条件嗷！')
        #printp()
        return False
    try:
        HJNUM=int(spin.get())
    except:
        tkinter.messagebox.showwarning("Info",'输入的获奖者数量没有意义呢！')
        return False
    if HJNUM<1:
        tkinter.messagebox.showwarning("Info",'输入的获奖者数量小于1，这是不想让小伙伴们抽中？')
        return False
    try:
        HJlvl=int(spin3.get())
    except:
        tkinter.messagebox.showwarning("Info",'输入的最低等级没有意义呢！')
        return False
    if HJlvl<0 or HJlvl>6:
        tkinter.messagebox.showwarning("Info",'输入的最低等级小于0或大于6！请不要调戏我呢…')
        return False
    try:
        CJHnum=int(spin2.get())
    except:
        tkinter.messagebox.showwarning("Info",'输入的过滤抽奖号的值没有意义嗷！')
        return False
    if CJHnum<-1 or CJHnum>10:
        tkinter.messagebox.showwarning("Info",'输入的过滤抽奖号的值小于-1或大于10！请不要调戏我呢…')
        return False
    if TRZ:
    ...
    if TGZ:
        try:
            ...
            try:
                cookiepath=askopenfilename(title='Choose a file which includes cookie',initialdir=os.path.dirname(os.path.realpath(sys.argv[0])), filetypes=[('Cookie File','*.txt')])
        ...
        if 'ENCRYPTED\n' in cookie:
            ...
            notime=True
            tkinter.messagebox.showwarning("Info",'需要解密cookie文件！')
    ...
    if TPL:
        if dyinfo['card']['desc']['comment']==0:
            printp('这条动态没有任何用户评论!')
            Error=True
        if dyinfo['card']['desc']['comment']>2000:
            printp('评论限制在2000条以内!')
            Error=True
...
def clicked2():
    #关于窗口
    tkinter.messagebox.showinfo("About", 'B站动态抽奖工具 Python GUI版\nFor Pydroid3 '+version+'\n更新日期: '+updatetime+'\nBy: 芍芋\nWebsite: https://shoyu.top')
...
def clicked3():
    ...
    login1window.title('Login/Cookie Actions')
...
def clicked4():
    ...
    qr = qrcode.QRCode(version=5, error_correction=(qrcode.constants.ERROR_CORRECT_M), box_size=12, border=4)
    qr.add_data(oauthlink)
    qr.make(fit=True)
    img = qr.make_image()
    img.save('qrcode.png')
    login2window = Toplevel(window)
    login2window.title('Scan QR with bilibili APP')
    width = 600
    heigh = 600
...
def clicked5():
    ...
    try:
        if os.path.exists('cookie.txt'):
            Isuse=tkinter.messagebox.askyesno("Info",'确认注销当前目录下的cookie？')
            if Isuse:
                cookiepath='cookie.txt'
        if cookiepath=='':
            cookiepath=askopenfilename(title='Choose a file which includes cookie',initialdir=os.path.dirname(os.path.realpath(sys.argv[0])), filetypes=[('Cookie File','*.txt')])
        ...
        tkinter.messagebox.showwarning("Info",'需要提供cookie才能注销掉呢……')
        return False
    #try:
    if 'ENCRYPTED\n' in cookie:
        #decrycook(cookiepath)
        #notime=True
        tkinter.messagebox.showwarning("Info",'请先将cookie文件解密！')
        decrycook(cookiepath)
        return False
    cookies_dict={}
    ...
    try:
        csrftoken=cookies_dict['bili_jct']
    	    ...
    	    tkinter.messagebox.showwarning("Info",'注销失败，指定的cookie无效！')
    	    return False
    ...
    try:
        ...
        tkinter.messagebox.showwarning("Info",'注销失败，可能是cookie无效、已过期或已注销！')
        return False
    if jdata['code']==0:
        if cookiepath=='cookie.txt':
            try:
                ...
                pass
        tkinter.messagebox.showinfo("Info",'文件包含的cookie已经成功从服务器上注销！')
        return True
    else:
        tkinter.messagebox.showwarning("Info",'注销cookie时出现其他错误！('+str(jdata['code'])+')')
        return False
...
def clicked6():
    ...
    try:
        ...
        if cookiepathz=='':
            cookiepathz=askopenfilename(title='Choose a file which includes cookie',initialdir=os.path.dirname(os.path.realpath(sys.argv[0])), filetypes=[('Cookie File','*.txt')])
        cook=open(cookiepathz,'r')
        cookiez=cook.read()
        cook.close()
    except:
        tkinter.messagebox.showwarning("Info",'需要提供cookie才能加密呢……')
        return False
    cook=open(cookiepathz,'r')
    cookiez=cook.read()
    cook.close()
    if 'ENCRYPTED\n' in cookiez:
        tkinter.messagebox.showwarning("Info",'这个cookie已经加密过了！')
        return False
    login3window = Toplevel(window)
    login3window.title('Input Password to Encrypt')
    width = 550
    heigh = 160
    ...
    try:
        ...
        try:
            ...
            pass
    txt2 = ttk.Entry(login3window, width=20)
...
def clicked7():
    ...
    outtxt.close()
    tkinter.messagebox.showinfo('Info','已将该cookie加密！')
...
def decrycook(cookiepath):
    ...
    login4window.title('Input Password to Decrypt')
    ...
    try:
        ...
        try:
            ...
            pass
    txt3 = ttk.Entry(login4window, width=20)
...
def clicked8():
    ...
    try:
        ...
        tkinter.messagebox.showwarning('Info','密码有误！')
        return False
    tkinter.messagebox.showinfo('Info','已将该cookie解密！')
    #IsSave=tkinter.messagebox.askyesno('Info','已将该cookie解密！是否需要保存？')
    #if IsSave:
...
def chklog(oauthkey,win):
    while True:
        if win.winfo_exists():
            ...
            jdata = json_dict
            if jdata['data']==-5:
                win.title('Need Confirm on app!')
            elif jdata['data']==-2:
                win.destroy()
                tkinter.messagebox.showwarning('Info','二维码已过期，请刷新！')
                break
            else:
                try:
                    if jdata['code']==0:
                        ...
                        outtxt.close()
                        win.destroy()
                        tkinter.messagebox.showinfo('Info','已将cookie写入同一目录下的cookie.txt！')
                except:
                        pass
        else:
            break
...
def clicked9():
    ...
    RZtxt.close()
    tkinter.messagebox.showinfo("Info",'抽奖记录已保存为：'+rzpath)

def clicked10(btn):
    global DisplayLogInfo
    DisplayLogInfo=bool(1-DisplayLogInfo)
    DPLI2=repBool2(DisplayLogInfo)
    #tkinter.messagebox.showinfo("Info",'已切换为：'+DPLI2)
...
lbl4.place(x=500, y=450)
lbl4.configure(bg='white')
'''lbl5 = Label(window, text="注: 评论获取不包括楼中楼")#\n抽取时如果数据过多可能会出现无响应，耐心等待即可~")
lbl5.place(x=10, y=186)
lbl5.configure(bg='white')'''
lbl6 = Label(window, text="最低等级")#\n(0-6)")
lbl6.place(x=10, y=553)
lbl6.configure(bg='white')
lbl7 = Label(window, text="值越小越严格,-1=无")
lbl7.place(x=650, y=550)
lbl7.configure(bg='white')
output = scrolledtext.ScrolledText(window, width=44, height=24, relief="solid")
output.place(x=9, y=1100)
output['state']='disabled'
bar = Progressbar(window, length=900)#290)
bar.place(x=10, y=872)
bar['value']=0
barp = Label(window, text="0%")
barp.place(x=938, y=871)
...
chkupdwindow.title('Check Update')
